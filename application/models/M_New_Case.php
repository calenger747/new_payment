...
class M_New_Case extends CI_Model{ 
    ...
    var $column_order_2 = array('new_cpv_list.cpv_number','client.full_name','bank.name','client.account_no','new_cpv_list.date_created','new_cpv_list.total_record','new_cpv_list.total_cover'); //set column field database for datatable orderable 
    var $column_search_2 = array('new_cpv_list.cpv_number','client.full_name','bank.name','client.account_no','new_cpv_list.date_created','new_cpv_list.total_record','cpv_list.total_cover'); //set column field database for datatable searchable 
    ...
    {
    	$query = $this->db->query("SELECT a.status, a.name FROM case_status AS a JOIN `case` AS b ON a.status = b.status JOIN new_history_batch_detail AS c ON b.id = c.case_id WHERE b.type = '$type' GROUP BY a.status ORDER BY a.status ASC");
    ...
    {
    	$query = $this->db->query("SELECT a.id AS id_client, a.full_name AS client_name FROM client AS a JOIN `case` AS b ON a.id = b.client WHERE b.id NOT IN (SELECT case_id FROM new_history_batch_detail WHERE status_batch !='9') AND b.type = '$type' AND b.status = '$status' GROUP BY a.id ORDER BY a.full_name ASC");
    ...
    public function get_client_batch($case_type, $case_status, $payment_by="", $tgl_batch="", $history_batch="", $source_bank="", $source_account="", $status_batch="", $user="")
    {
    ...
    public function get_source_bank($case_type, $case_status, $payment_by="", $status_batch="", $user="")
    {
        // $query = $this->db->query("SELECT keterangan FROM history_batch WHERE DATE_FORMAT(tgl_batch, '%Y-%m-%d') = '$tgl_batch' AND type = '$type' ");
    ...
    public function get_source_account($case_type, $case_status, $payment_by="", $source_bank="", $status_batch="", $user="")
    {
        // $query = $this->db->query("SELECT keterangan FROM history_batch WHERE DATE_FORMAT(tgl_batch, '%Y-%m-%d') = '$tgl_batch' AND type = '$type' ");
    ...
    public function get_beneficiary_bank($case_type, $case_status, $payment_by="", $source_bank="", $source_account="", $status_batch="", $user="")
    {
        // $query = $this->db->query("SELECT keterangan FROM history_batch WHERE DATE_FORMAT(tgl_batch, '%Y-%m-%d') = '$tgl_batch' AND type = '$type' ");
    ...
    public function get_beneficiary_account($case_type, $case_status, $payment_by="", $source_bank="", $source_account="", $beneficiary_bank="", $status_batch="", $user="")
    {   
    ...
    {   
    	...
    	$this->db->where('case_status.`status` = "'.$this->input->post('status').'"');
    	$this->db->where('new_history_batch_detail.status_batch != "9"');
    ...
    {   
    	...
    	$this->db->where('new_history_batch_detail.status_batch', '1');
